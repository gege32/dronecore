...
#include "commdriver/WifiConfig.h"
...
bool	Wifi_SendRaw(uint8_t *data,uint16_t len)
{
	if(len <= _WIFI_TX_SIZE)
	{
		...
		if(HAL_UART_Transmit(uart_handle,data,len,100) == HAL_OK)
			return true;
		else
			return false;
	}
...
void	Wifi_RxClear(void)
{
	...
	Wifi.RxIndex=0;	
  HAL_UART_Receive_IT(uart_handle,&Wifi.usartBuff,1);
}
//#########################################################################################################
...
//#########################################################################################################
//#########################################################################################################
void	Wifi_Init(osPriority	Priority, UART_HandleTypeDef* huart)
{
    uart_handle = huart;
	HAL_UART_Receive_IT(uart_handle,&Wifi.usartBuff,1);
	Wifi_RxClear();
